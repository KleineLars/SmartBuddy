...
window.__cachedSmscName = 'Gast';
  ...
  function createOverlay() {
    ...
    overlay.querySelector('#sg-chat-btn').addEventListener('click', async (e) => {
      ...
      setTimeout(() => {
          ...
          if (!iframe.src || iframe.src === window.location.href || iframe.src === 'about:blank') {
              const urlObj = new URL(chrome.runtime.getURL('pages/smartchat.html'));
              urlObj.searchParams.set('player_name', currentGlobalName);
    ...
    playBtnBloob.addEventListener('click', (e) => { e.stopPropagation(); openGame(chrome.runtime.getURL('games/smartbloob-io.html'), 'SmartBloob.io', 'SmartBloob-io'); });
    cardBloob.addEventListener('click', () => openGame(chrome.runtime.getURL('games/smartbloob-io.html'), 'SmartBloob.io', 'SmartBloob-io'));
    
    playBtnWave.addEventListener('click', (e) => { e.stopPropagation(); openGame(chrome.runtime.getURL('games/smartwave-io.html'), 'SmartWave.io', 'smartwave-io'); });
    cardWave.addEventListener('click', () => openGame(chrome.runtime.getURL('games/smartwave-io.html'), 'SmartWave.io', 'smartwave-io'));

    playBtnPong.addEventListener('click', (e) => { e.stopPropagation(); openGame(chrome.runtime.getURL('games/smartpong-io.html'), 'SmartPong.io', 'smartpong-io'); });
    cardPong.addEventListener('click', () => openGame(chrome.runtime.getURL('games/smartpong-io.html'), 'SmartPong.io', 'smartpong-io'));
