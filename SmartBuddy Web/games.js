...
window.__cachedSmscName = 'Gast';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      #sg-mod-btn:hover { background: rgba(236, 72, 153, 0.1); border-color: rgba(236, 72, 153, 0.3); color: #ec4899; transform: scale(1.1); }
  ...
  function createOverlay() {
    ...
    function openOverlay() {
      sendMessageToParent({ action: 'getApiKey' }).then((apiKey) => {
        ...
        overlay.querySelector('#sg-chat-btn').style.display = 'none';
          ...
          overlay.querySelector('#sg-apikey-close').style.display = 'none';
        return;
        } else {
    ...
    function closeOverlay() {
      ...
      setTimeout(() => {
          ...
          if (chatView && chatView.style.display !== 'none') {
              ...
              if (contentBox) { contentBox.style.padding = ''; contentBox.style.height = ''; }
              const closeBtn = overlay.querySelector('#sg-games-close');
    ...
    overlay.querySelector('#sg-apikey-save').addEventListener('click', async () => {
      ...
      chrome.runtime.sendMessage({ action: 'testGeminiKey', key: val }, (res) => {
          if (res && res.success) {
              sendMessageToParent({ action: 'setApiKey', value: val }).then(() => {
    ...
    overlay.querySelector('#sg-chat-btn').addEventListener('click', async (e) => {
      ...
      setTimeout(() => {
          ...
          if (contentBox) { contentBox.style.padding = '0'; contentBox.style.height = currentHeight + 'px'; }
